...
class Combination(CombObject):
    ...
    def _Rank(self, v, n, m):
        if m == 0 or m == n:
            ...
            if v[0] == 0:
                ...
                r = self._Rank(v[1:], n - 1, m - 1) + self.C_Cardinality(n - 1, m)
        return r
    ...
    def _Unrank(self, r, n, m):
        if m == 0 or m == n:
            ...
            if r < self.C_Cardinality(n - 1, m):
                v = [0] + self._Unrank(r, n - 1, m)
            else:
                v = [1] + self._Unrank(r - self.C_Cardinality(n - 1, m), n - 1, m - 1)
        return v

"""
a = Combination(n = 5, m = 2)
